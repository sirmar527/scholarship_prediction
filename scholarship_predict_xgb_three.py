...
FEATURE_DESCRIPTIONS = {
    ...
    "prev_scholarship": "была ли стипендия в текущем семестре",
}
...
def _save_human_report(
    ...
    w(
        "Каждый студент попадает в одну из четырёх категорий в зависимости от того, "
        "была ли у него стипендия сейчас и будет ли в следующем семестре:\n"
    )
    for cls in [1, 2, 3, 4]:
        if cls in class_b_accuracies:
            w(
    ...
    if actual_lost > 0:
        w(
            f"**Потеря стипендии:** из {actual_lost:,} студентов, реально потерявших "
            f"стипендию, модель верно предупредила о {caught_lost:,} "
            f"({caught_lost / actual_lost:.0%}). "
        )
    if actual_gained > 0:
        w(
            f"**Получение стипендии:** из {actual_gained:,} студентов, впервые получивших "
            f"стипендию, модель верно предсказала {caught_gained:,} "
    ...
    w(
        "Каждому студенту модель присваивает вероятность получения стипендии "
        "(от 0% до 100%). На основе этой вероятности студенты делятся на группы:\n"
    )
    w(
