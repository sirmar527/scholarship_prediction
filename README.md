...
Для пары (семестр N, семестр N+1) одного студента предсказать, будет ли семестр N+1 чистым (без блокирующих оценок и без пересдач). По стандартным академическим правилам чистый семестр N+1 даёт право на стипендию в семестре N+2; модель работает как ранний прогноз на один семестр вперёд.
...
Лучший бейзлайн на той же выборке: accuracy ~82.3 % (по правилу "следующий семестр будет таким же, как текущий"). Полные таблицы (confusion matrix, разбивка по семестрам, 4-классовая транзиция Чисто/Провал × Чисто/Провал) находятся в `outputs/histgb/summary.md`.
