...
# =====================================================================
# 2. FEATURE ENGINEERING  (fixed: pre-aggregate flags before dedup)
# =====================================================================
def build_features(df):
    ...
    disc_flags = df.groupby(disc_key).agg(
        disc_any_fail=("is_fail", "max"),
    ...
    df["is_fail"] = df["disc_any_fail"]
    df["has_retake"] = df["disc_any_retake"]
    df["grade_num"] = df[["grade_num", "disc_min_grade"]].min(axis=1)
    df = df.drop(columns=["disc_any_fail", "disc_any_retake", "disc_min_grade"])
...
def _prepare_features(pairs_df):
    ...
    for col in X.columns:
        if X[col].isna().any():
            if col in ("gpa_overall", "min_grade", "share_5"):
...
def evaluate_and_save(model, best_iter, X, y, feature_cols,
    ...
    for from_s in sorted(set(test_sems)):
        ...
        if len(set(y_true_s)) > 1:
            ...
            auc_s = 0
        print(f"  {int(from_s)}→{int(from_s) + 1}: "
...
def main():
    ...
    parser.add_argument("--output", type=str, default="output_single",
                        help="Папка для результатов (default: output_single/)")
